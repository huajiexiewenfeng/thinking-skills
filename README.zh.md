...
2. 在自己的写作、技术讨论、情绪梳理、创意探索或决策场景中使用它。
...
| 层级 | 责任 |
|---|---|
| Router | 判断用户意图并选择合适的思考模式 |
| Domain skills | 用领域专属判断标准和输出形态处理真实任务 |
| Method bases | 显式声明方法底座，但不默认把框架倒给用户 |
| Meta skills | 复盘 skill 触发轨迹、模式切换、失败信号和 eval 缺口 |
| Improvement loop | 把真实失败转化为抽象案例、eval 和小补丁 |
| Platform adapters | 在不同 agent runtime 中暴露同一份 canonical skills |
